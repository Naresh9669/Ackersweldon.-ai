...
interface QuoteSummary {
  summaryDetail?: FinanceData;
  defaultKeyStatistics?: FinanceData;
  [key: string]: any;
}
...
export default async function FinanceSummary({ ticker }: { ticker: string }) {
  const financeSummaryData: QuoteSummary | null = await fetchQuoteSummary(ticker)
  ...
  return (
    <div className="grid grid-flow-col grid-rows-6 gap-4 md:grid-rows-3">
      {keysToDisplay.map((item) => {
        ...
        // Fix: Use safe property access with type guards
        const sectionData = financeSummaryData?.[section] as FinanceData | undefined
