    ...
    try:
        ...
        for article in articles:
            ...
            processed_article = {
                ...
                "ai_summary_timestamp": article.get("ai_summary_timestamp", ""),
                "ai_summary_model": article.get("ai_summary_model", ""),
                "ai_sentiment_analysis": article.get("ai_sentiment_analysis", ""),
                "ai_sentiment_timestamp": article.get("ai_sentiment_timestamp", ""),
                "ai_sentiment_model": article.get("ai_sentiment_model", ""),
                # Enhanced Sentiment Fields
                "sentiment_confidence": article.get("sentiment_confidence", 0),
                "sentiment_analysis_date": article.get("sentiment_analysis_date", ""),
                '_id': article['_id'],
...
# AI Results Update API
@app.route('/api/news/ai-update', methods=['POST'])
def update_news_ai_results():
    """Update news article with AI analysis results"""
    try:
        data = request.get_json()
        
        if not data or 'article_id' not in data or 'ai_type' not in data:
            return jsonify({
                'success': False,
                'error': 'Missing required fields: article_id, ai_type'
            }), 400
        ...
        # Prepare update data based on AI type
        update_data = {}
        ...
        if ai_type == 'summary':
            if 'content' not in data:
                return jsonify({
                    'success': False,
                    'error': 'Missing content for summary'
                }), 400
            
            update_data.update({
                'ai_summary': data['content'],
                'ai_summary_timestamp': timestamp,
                'ai_summary_model': data.get('model', 'unknown'),
                'ai_summary_metadata': data.get('metadata', {})
            })
            
        elif ai_type == 'sentiment':
            if 'content' not in data:
                return jsonify({
                    'success': False,
                    'error': 'Missing content for sentiment'
                }), 400
            
            update_data.update({
                'ai_sentiment_analysis': data['content'],
                'ai_sentiment_timestamp': timestamp,
                'ai_sentiment_model': data.get('model', 'unknown'),
                'ai_sentiment_metadata': data.get('metadata', {}),
                'sentiment_confidence': data.get('confidence', 0),
                'sentiment_analysis_date': timestamp
            })
            
        else:
            return jsonify({
                'success': False,
                'error': 'Invalid ai_type. Must be "summary" or "sentiment"'
            }), 400
        ...
        if result.modified_count == 0:
            return jsonify({
                'success': False,
                'error': 'No changes made to article'
            }), 400
        
        logger.info(f"Updated article {article_id} with {ai_type} AI results")
        
        return jsonify({
            'success': True,
            'message': f'AI {ai_type} results updated successfully',
        ...
        logger.error(f"Error updating AI results: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
# Get AI Results for Article
@app.route('/api/news/<article_id>/ai-results', methods=['GET'])
def get_article_ai_results(article_id):
    """Get AI analysis results for a specific news article"""
    try:
        # Validate article_id format
        try:
            from bson import ObjectId
        ...
        # Find the article
        article = db.news_metadata.find_one({'_id': object_id})
        ...
        ai_results = {
            ...
            'ai_summary': {
                ...
                'timestamp': article.get('ai_summary_timestamp', ''),
                'model': article.get('ai_summary_model', ''),
                'metadata': article.get('ai_summary_metadata', {})
            },
            'ai_sentiment': {
                'content': article.get('ai_sentiment_analysis', ''),
                'timestamp': article.get('ai_sentiment_timestamp', ''),
                'model': article.get('ai_sentiment_model', ''),
                'metadata': article.get('ai_sentiment_metadata', {}),
        ...
        logger.error(f"Error retrieving AI results: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
# AI Caching Management Endpoints
# These replace the frontend /api/ai/store functionality

@app.route('/api/ai/cache', methods=['POST'])
def store_ai_cache():
    """Store AI results in cache (replaces frontend /api/ai/store POST)"""
    try:
        data = request.get_json()
        article_id = data.get('articleId')
        ai_type = data.get('type')  # 'summary' or 'sentiment'
        content = data.get('content')
        success = data.get('success', True)
        metadata = data.get('metadata', {})
        
        if not all([article_id, ai_type, content]):
            return jsonify({
                'success': False,
                'error': 'Missing required fields: articleId, type, content'
            }), 400
        
        if ai_type not in ['summary', 'sentiment']:
            return jsonify({
                'success': False,
                'error': 'Invalid ai_type. Must be "summary" or "sentiment"'
            }), 400
        ...
        if ai_type == 'summary':
            update_data = {
                'ai_summary': content,
                'ai_summary_timestamp': timestamp.isoformat(),
                'ai_summary_model': metadata.get('model', 'unknown'),
                'ai_summary_metadata': metadata
            }
        else:  # sentiment
            update_data = {
                'ai_sentiment': content,  # Use the field name from existing backend
                'ai_sentiment_created_at': timestamp.isoformat(),  # Use the field name from existing backend
                'ai_sentiment_model': metadata.get('model', 'unknown'),  # Use the field name from existing backend
                'ai_sentiment_confidence': metadata.get('confidence', 0)  # Use the field name from existing backend
            }
        ...
        logger.info(f"Stored AI {ai_type} cache for article {article_id}")
        
        return jsonify({
            'success': True,
            'message': f'AI {ai_type} cached successfully',
            'key': f"{article_id}_{ai_type}",
            'cached': True,
            'timestamp': timestamp.isoformat()
        })
        
    except Exception as e:
        logger.error(f"Error storing AI cache: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def get_ai_cache():
    """Retrieve AI results from cache (replaces frontend /api/ai/store GET)"""
    try:
        article_id = request.args.get('articleId')
        ai_type = request.args.get('type')  # 'summary' or 'sentiment'
        
        if not all([article_id, ai_type]):
            return jsonify({
                'success': False,
                'error': 'Missing required parameters: articleId, type'
            }), 400
        ...
        # Find the article and extract AI results
        article = db.news_metadata.find_one({'_id': object_id})
        ...
        # Extract the requested AI result
        if ai_type == 'summary':
            ai_content = article.get('ai_summary')
            ai_timestamp = article.get('ai_summary_timestamp')
            ai_model = article.get('ai_summary_model')
            ai_metadata = article.get('ai_summary_metadata', {})
        else:  # sentiment
            ai_content = article.get('ai_sentiment')  # Use the field name from existing backend
            ai_timestamp = article.get('ai_sentiment_created_at')  # Use the field name from existing backend
            ai_model = article.get('ai_sentiment_model')  # Use the field name from existing backend
            ai_metadata = {}  # No metadata field in existing backend
        
        if not ai_content:
            return jsonify({
                'success': False,
                'error': f'No cached {ai_type} found for this article'
            }), 404
        ...
        logger.error(f"Error retrieving AI cache: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
@app.route('/api/ai/cache/bulk', methods=['POST'])
def bulk_get_ai_cache():
    """Retrieve AI results for multiple articles at once (for frontend preloading)"""
    try:
        data = request.get_json()
        article_ids = data.get('articleIds', [])
        ai_type = data.get('type')  # 'summary' or 'sentiment'
        
        if not article_ids or not ai_type:
            return jsonify({
                'success': False,
                'error': 'Missing required fields: articleIds, type'
            }), 400
        ...
        # Validate article_ids format
        try:
            object_ids = [ObjectId(aid) for aid in article_ids]
        except:
            return jsonify({
                'success': False,
                'error': 'Invalid article_id format in articleIds array'
            }), 400
        
        # Find articles and extract AI results
        articles = list(db.news_metadata.find(
            {'_id': {'$in': object_ids}},
            {'_id': 1, 'ai_summary': 1, 'ai_summary_timestamp': 1, 'ai_summary_model': 1, 'ai_summary_metadata': 1,
             'ai_sentiment': 1, 'ai_sentiment_created_at': 1, 'ai_sentiment_model': 1, 'ai_sentiment_confidence': 1}
        ))
        ...
        logger.error(f"Error bulk retrieving AI cache: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def clear_ai_cache():
    """Clear AI cache for specific articles or all articles"""
    try:
        data = request.get_json()
        article_ids = data.get('articleIds', [])  # Empty array means clear all
        ai_type = data.get('type')  # 'summary', 'sentiment', or 'all'
        
        if ai_type not in ['summary', 'sentiment', 'all']:
            return jsonify({
                'success': False,
                'error': 'Invalid ai_type. Must be "summary", "sentiment", or "all"'
            }), 400
        
        # Prepare update data to clear AI fields
        if ai_type == 'summary':
            clear_fields = {
                'ai_summary': '',
            ...
            clear_fields = {
                'ai_sentiment': '',  # Use the field name from existing backend
                'ai_sentiment_created_at': '',  # Use the field name from existing backend
                'ai_sentiment_model': '',  # Use the field name from existing backend
                'ai_sentiment_confidence': 0  # Use the field name from existing backend
            }
        else:  # all
            clear_fields = {
                'ai_summary': '',
        ...
        if article_ids:
            ...
            query = {}  # Clear all articles
        
        # Update database
        result = db.news_metadata.update_many(query, {'$set': clear_fields})
        
        logger.info(f"Cleared AI {ai_type} cache for {result.modified_count} articles")
        
        return jsonify({
            'success': True,
            'message': f'AI {ai_type} cache cleared successfully',
            'articles_modified': result.modified_count
        })
